...
class PayableBottomSheetViewController: BottomSheetViewController, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        setupTableView()
        fetchPayableSettlements()
    }

    private func setupTableView() {
        tableView.dataSource = self
        tableView.register(UITableViewCell.self, forCellReuseIdentifier: "PayableCell")
    ...
    private func fetchPayableSettlements() {
        NetworkManager.shared.getSettlementsPayable(tripId: tripId) { [weak self] result in
            DispatchQueue.main.async {
                ...
                case .failure(let error):
                    print("Failed to fetch payable settlements: \(error)")
                }
            }
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "PayableCell", for: indexPath)
